...
"""
Отступы между классами и отдельными функциями - в две строки. 
Внутри класса между методами - одна строка.
"""
...
def data_files_tasks():
    ...
    if size > 0:
        ...
        # tasks_data = json.dumps(tasks_data, default=str)
        # tasks_data = json.loads(str(tasks_data))
...
def main(choice=None):
    ...
    if choice == "1":
        ...
        if password != password1:
            ...
            sign_up(username, password, uid=None)
    elif choice == "3":
        exitt()
...
def task_manager(username, password):
    ...
    if choice == '1':
        all_tasks(username, password)
    elif choice == '2':
        add_task(username, password)
    elif choice == '3':
        tasks_status(username, password)
    elif choice == '4':
        update_status(username, password)
    elif choice == '5':
        main()
    os.system('cls')
...
def add_task(username, password):
    ...
    with open('datafiles/datatasks.json', 'r') as file:
        for i in tasks_data:
            ...
            tasks_data["userstasks"].append(Task(username, date, content, steps, status).__dict__)
            with open("datafiles/datatasks.json", 'w') as outfile:
                json.dump(tasks_data, outfile, indent=4, default=str)
    task_manager(username, password)


# не доделаное
"""tasks_data["userstasks"].append({tid : []})
                            for i in tasks_data['userstasks']:
                                val = i.values()
                                for key, val in i:
                                    print(val)
                                    #val.append(Task(username, date, content, steps, status))
                            #tasks_data["userstasks"].append(Task(username, date, content, steps, status).__dict__)
                            with open("datafiles/datatasks.json", 'w') as outfile:
                                json.dump(tasks_data, outfile, indent=4, default=str)"""
...
def tasks_status(username, password):
    ...
    with open("datafiles/datatasks.json") as f:
        tasks_data = json.load(f)
        if status_choice == '1':
            if compare_data(username, password) == True:
                for i in tasks_data['userstasks']:
                    d = time.localtime()
...
def sign_up(username, password, uid=None):
    ...
    with open('datafiles/data.json', 'r') as file:
        ...
        if user_exists(username, password) == True:
            ...
            for i in data:
                data['userinfo'].append(User(username, password, uid).__dict__)
